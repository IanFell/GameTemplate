package loaders.plantloaders;

import loaders.GameObjectLoader;

/**
 * 
 * @author Fabulous Fellini
 *
 */
public class PlantLoader extends GameObjectLoader {
	
	private PlantLoaderMapChunkTwo plantLoaderMapChunkTwo;
	private PlantLoaderMapChunkThree plantLoaderMapChunkThree;
	private PlantLoaderMapChunkFour plantLoaderMapChunkFour;
	private PlantLoaderMapChunkFive plantLoaderMapChunkFive;
	private PlantLoaderMapChunkSix plantLoaderMapChunkSix;
	private PlantLoaderMapChunkSeven plantLoaderMapChunkSeven;
	private PlantLoaderMapChunkEight plantLoaderMapChunkEight;
	private PlantLoaderMapChunkEleven plantLoaderMapChunkEleven;
	private PlantLoaderMapChunkTwelve plantLoaderMapChunkTwelve;
	private PlantLoaderMapChunkThirteen plantLoaderMapChunkThirteen;
	private PlantLoaderMapChunkFourteen plantLoaderMapChunkFourteen;
	private PlantLoaderMapChunkFifteen plantLoaderMapChunkFifteen;
	private PlantLoaderMapChunkSixteen plantLoaderMapChunkSixteen;
	private PlantLoaderMapChunkTwenty plantLoaderMapChunkTwenty;
	private PlantLoaderMapChunkTwentyOne plantLoaderMapChunkTwentyOne;
	private PlantLoaderMapChunkTwentyTwo plantLoaderMapChunkTwentyTwo;
	private PlantLoaderMapChunkTwentyThree plantLoaderMapChunkTwentyThree;
	private PlantLoaderMapChunkTwentyFour plantLoaderMapChunkTwentyFour;
	private PlantLoaderMapChunkTwentySix plantLoaderMapChunkTwentySix;
	private PlantLoaderMapChunkTwentyEight plantLoaderMapChunkTwentyEight;
	private PlantLoaderMapChunkTwentyNine plantLoaderMapChunkTwentyNine;
	private PlantLoaderMapChunkThirty plantLoaderMapChunkThirty;
	private PlantLoaderMapChunkThirtyOne plantLoaderMapChunkThirtyOne;
	private PlantLoaderMapChunkThirtyTwo plantLoaderMapChunkThirtyTwo;
	private PlantLoaderMapChunkThirtySeven plantLoaderMapChunkThirtySeven;
	private PlantLoaderMapChunkThirtyEight plantLoaderMapChunkThirtyEight;
	private PlantLoaderMapChunkThirtyNine plantLoaderMapChunkThirtyNine;
	private PlantLoaderMapChunkFourty plantLoaderMapChunkFourty;
	private PlantLoaderMapChunkFourtyThree plantLoaderMapChunkFourtyThree;
	private PlantLoaderMapChunkFourtyFive plantLoaderMapChunkFourtyFive;
	private PlantLoaderMapChunkFourtySix plantLoaderMapChunkFourtySix;
	private PlantLoaderMapChunkFourtySeven plantLoaderMapChunkFourtySeven;
	private PlantLoaderMapChunkFourtyEight plantLoaderMapChunkFourtyEight;
	private PlantLoaderMapChunkFiftyOne plantLoaderMapChunkFiftyOne;
	private PlantLoaderMapChunkFiftyTwo plantLoaderMapChunkFiftyTwo;
	private PlantLoaderMapChunkFiftyThree plantLoaderMapChunkFiftyThree;
	private PlantLoaderMapChunkFiftyFour plantLoaderMapChunkFiftyFour;
	private PlantLoaderMapChunkFiftyFive plantLoaderMapChunkFiftyFive;
	private PlantLoaderMapChunkFiftySix plantLoaderMapChunkFiftySix;
	
	public PlantLoader() {
		plantLoaderMapChunkTwo         = new PlantLoaderMapChunkTwo();
		plantLoaderMapChunkThree       = new PlantLoaderMapChunkThree();
		plantLoaderMapChunkFour        = new PlantLoaderMapChunkFour();
		plantLoaderMapChunkFive        = new PlantLoaderMapChunkFive();
		plantLoaderMapChunkSix         = new PlantLoaderMapChunkSix();
		plantLoaderMapChunkSeven       = new PlantLoaderMapChunkSeven();
		plantLoaderMapChunkEight       = new PlantLoaderMapChunkEight();
		plantLoaderMapChunkEleven      = new PlantLoaderMapChunkEleven();
		plantLoaderMapChunkTwelve      = new PlantLoaderMapChunkTwelve();
		plantLoaderMapChunkThirteen    = new PlantLoaderMapChunkThirteen();
		plantLoaderMapChunkFourteen    = new PlantLoaderMapChunkFourteen();
		plantLoaderMapChunkFifteen     = new PlantLoaderMapChunkFifteen();
		plantLoaderMapChunkSixteen     = new PlantLoaderMapChunkSixteen();
		plantLoaderMapChunkTwenty      = new PlantLoaderMapChunkTwenty();
		plantLoaderMapChunkTwentyOne   = new PlantLoaderMapChunkTwentyOne();
		plantLoaderMapChunkTwentyTwo   = new PlantLoaderMapChunkTwentyTwo();
		plantLoaderMapChunkTwentyThree = new PlantLoaderMapChunkTwentyThree();
		plantLoaderMapChunkTwentyFour  = new PlantLoaderMapChunkTwentyFour();
		plantLoaderMapChunkTwentySix   = new PlantLoaderMapChunkTwentySix();
		plantLoaderMapChunkTwentyEight = new PlantLoaderMapChunkTwentyEight();
		plantLoaderMapChunkTwentyNine  = new PlantLoaderMapChunkTwentyNine();
		plantLoaderMapChunkThirty      = new PlantLoaderMapChunkThirty();
		plantLoaderMapChunkThirtyOne   = new PlantLoaderMapChunkThirtyOne();
		plantLoaderMapChunkThirtyTwo   = new PlantLoaderMapChunkThirtyTwo();
		plantLoaderMapChunkThirtySeven = new PlantLoaderMapChunkThirtySeven();
		plantLoaderMapChunkThirtyEight = new PlantLoaderMapChunkThirtyEight();
		plantLoaderMapChunkThirtyNine  = new PlantLoaderMapChunkThirtyNine();
		plantLoaderMapChunkFourty      = new PlantLoaderMapChunkFourty();
		plantLoaderMapChunkFourtyThree = new PlantLoaderMapChunkFourtyThree();
		plantLoaderMapChunkFourtyFive  = new PlantLoaderMapChunkFourtyFive();
		plantLoaderMapChunkFourtySix   = new PlantLoaderMapChunkFourtySix();
		plantLoaderMapChunkFourtySeven = new PlantLoaderMapChunkFourtySeven();
		plantLoaderMapChunkFourtyEight = new PlantLoaderMapChunkFourtyEight();
		plantLoaderMapChunkFiftyOne    = new PlantLoaderMapChunkFiftyOne();
		plantLoaderMapChunkFiftyTwo    = new PlantLoaderMapChunkFiftyTwo();
		plantLoaderMapChunkFiftyThree  = new PlantLoaderMapChunkFiftyThree();
		plantLoaderMapChunkFiftyFour   = new PlantLoaderMapChunkFiftyFour();
		plantLoaderMapChunkFiftyFive   = new PlantLoaderMapChunkFiftyFive();
		plantLoaderMapChunkFiftySix    = new PlantLoaderMapChunkFiftySix();
		loadPlants();
	}
	
	public void loadPlants() {
		plantLoaderMapChunkTwo.loadPlants();
		plantLoaderMapChunkThree.loadPlants();
		plantLoaderMapChunkFour.loadPlants();
		plantLoaderMapChunkFive.loadPlants();
		plantLoaderMapChunkSix.loadPlants();
		plantLoaderMapChunkSeven.loadPlants();
		plantLoaderMapChunkEight.loadPlants();
		plantLoaderMapChunkEleven.loadPlants();
		plantLoaderMapChunkTwelve.loadPlants();
		plantLoaderMapChunkThirteen.loadPlants();
		plantLoaderMapChunkFourteen.loadPlants();
		plantLoaderMapChunkFifteen.loadPlants();
		plantLoaderMapChunkSixteen.loadPlants();
		plantLoaderMapChunkTwenty.loadPlants();
		plantLoaderMapChunkTwentyOne.loadPlants();
		plantLoaderMapChunkTwentyTwo.loadPlants();
		plantLoaderMapChunkTwentyThree.loadPlants();
		plantLoaderMapChunkTwentyFour.loadPlants();
		plantLoaderMapChunkTwentySix.loadPlants();
		plantLoaderMapChunkTwentyEight.loadPlants();
		plantLoaderMapChunkTwentyNine.loadPlants();
		plantLoaderMapChunkThirty.loadPlants();
		plantLoaderMapChunkThirtyOne.loadPlants();
		plantLoaderMapChunkThirtyTwo.loadPlants();
		plantLoaderMapChunkThirtySeven.loadPlants();
		plantLoaderMapChunkThirtyEight.loadPlants();
		plantLoaderMapChunkThirtyNine.loadPlants();
		plantLoaderMapChunkFourty.loadPlants();
		plantLoaderMapChunkFourtyThree.loadPlants();
		plantLoaderMapChunkFourtyFive.loadPlants();
		plantLoaderMapChunkFourtySix.loadPlants();
		plantLoaderMapChunkFourtySeven.loadPlants();
		plantLoaderMapChunkFourtyEight.loadPlants();
		plantLoaderMapChunkFiftyOne.loadPlants();
		plantLoaderMapChunkFiftyTwo.loadPlants();
		plantLoaderMapChunkFiftyThree.loadPlants();
		plantLoaderMapChunkFiftyFour.loadPlants();
		plantLoaderMapChunkFiftyFive.loadPlants();
		plantLoaderMapChunkFiftySix.loadPlants();
	}
}
